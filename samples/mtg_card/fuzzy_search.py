...
# %%
set_names = {x.lower():y for x,y in cards.groupby('name').apply(get_unique_set_names).to_dict().items() }
expanded_names = {x.lower():y for x,y in cards.groupby('name').apply(get_unique_set_names).to_dict().items() }
...
def load_set_image(url, width=None, height=None):
    ...
    png_data = svg2png(bytestring=svg_data, output_width=width-border_size*2, output_height=height-border_size*2)
    # Decode the PNG
    ...
    cv_img = cv2.copyMakeBorder(cv_img, border_size, border_size, border_size, border_size, cv2.BORDER_CONSTANT, value=border_color)
    return cv_img
